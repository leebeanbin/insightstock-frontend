/**
 * StockHoverPreview Component
 * 종목 호버 시 표시되는 미리보기 차트 컴포넌트 (토스증권 스타일)
 */
...
interface StockHoverPreviewProps {
  stock: Stock;
  position: { x: number; y: number };
  onClose: () => void;
}

export function StockHoverPreview({ stock, position, onClose }: StockHoverPreviewProps) {
  ...
  return (
    <div
      className={cn(
        'fixed z-50 bg-white dark:bg-gray-800 rounded-lg shadow-2xl border border-gray-200 dark:border-gray-700',
        'w-[500px] max-h-[600px] overflow-y-auto',
    ...
    >
      <div className="p-4">
        ...
        <div className="flex items-center justify-between mb-3 pb-3 border-b border-gray-200 dark:border-gray-700">
          ...
          <div className="text-right">
            ...
            <div className="flex items-center justify-end gap-1 mt-1">
              <TrendIcon size={14} className={isPositive ? 'text-red-500' : 'text-blue-500'} />
              <span className={cn('text-sm font-bold', isPositive ? 'text-red-500' : 'text-blue-500')}>
        ...
        {/* 종목 정보 */}
        <div className="grid grid-cols-2 gap-3 mb-3 pb-3 border-b border-gray-200 dark:border-gray-700 text-xs">
          {stock.volume && (
            <div>
